...
@AndroidEntryPoint
class MultiLvlFragmentTwo : BaseMultiFragment<FragmentMultiLevelBinding>(FragmentMultiLevelBinding::inflate), View.OnClickListener {

    override val numberRound = 2

    private val viewModel by viewModels<MultiLevelViewModel>()

    private val safeArgs: MultiLvlFragmentTwoArgs by navArgs()

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)

        val idMultiLvl = safeArgs.idLvlMulit
        ...
        val callbackBackPressedCallback = requireActivity().onBackPressedDispatcher.addCallback(viewLifecycleOwner){
            val action = MultiLvlFragmentTwoDirections.actionMultiLvlFragmentFragmentTwoToMainMenuFragment()
    ...
    override fun onClick(v: View) {
        ...
        if(idButtonP1 > 0 && idButtonP2 > 0) {
            ...
            var idMultiLvl = safeArgs.idLvlMulit

            val nextLevel = ++idMultiLvl
            ...
            viewModel.roundMusicLivedata.observe(viewLifecycleOwner) { music ->
                ...
                mediaPlayer!!.setOnCompletionListener {
                    ...
                    GlobalScope.launch(Dispatchers.Main) {
                        releaseMediaPlayer()
                        delay(DELAY)
                        val action = MultiLvlFragmentTwoDirections.actionMultiLvlFragmentFragmentTwoToMultiLvlFragmentFragmentThree(nextLevel, scoreP1, scoreP2)
                        findNavController().navigate(action)
                    }
