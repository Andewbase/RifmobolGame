...
@AndroidEntryPoint
class MultiLvlFragmentThree : BaseMultiFragment<FragmentMultiLevelBinding>(FragmentMultiLevelBinding::inflate), View.OnClickListener {

    override val numberRound = 3

    private val viewModel by viewModels<MultiLevelViewModel>()

    private val safeArgs: MultiLvlFragmentThreeArgs by navArgs()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val callbackBackPressedCallback = requireActivity().onBackPressedDispatcher.addCallback(viewLifecycleOwner){
            if (Yodo1Mas.getInstance().isInterstitialAdLoaded){
                ...
                val action = MultiLvlFragmentThreeDirections.actionMultiLvlFragmentFragmentThreeToMainMenuFragment()
    ...
    override fun onClick(v: View) {
        ...
        if(idButtonP1 > 0 && idButtonP2 > 0) {
            ...
            var idMultiLvl = safeArgs.idMultiLvl
            ...
            viewModel.roundMusicLivedata.observe(viewLifecycleOwner) { music ->
                ...
                mediaPlayer!!.setOnCompletionListener {
                    ...
                    GlobalScope.launch(Dispatchers.Main) {
                        releaseMediaPlayer()
                        delay(DELAY)
                        val action = MultiLvlFragmentThreeDirections.actionMultiLvlFragmentFragmentThreeToMultiLvlFragmentFragmentFour(nextLevel, scoreP1, scoreP2)
                        findNavController().navigate(action)
                    }
                } //MediaPlayer.setOnCompletionListener
            } // viewModel.observe
