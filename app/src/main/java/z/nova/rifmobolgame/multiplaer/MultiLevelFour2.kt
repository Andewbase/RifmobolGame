   ...
   lateinit var p2btn_bottom_left: Button
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btn_back.setOnClickListener {
            ...
            if (Yodo1Mas.getInstance().isInterstitialAdLoaded) { //TODO
                Yodo1Mas.getInstance().showInterstitialAd(this@MultiLevelFour2) //Показать рекламу
                val intent = Intent(
                    this@MultiLevelFour2,
                    GameMenu::class.java
        ...
        p1btn_up_left.setOnClickListener(View.OnClickListener {
            try {
                ...
                if (check == 2) { //Если нажали два раза
                    ...
                    if (mediaPlayer != null) {
                        releaseMP()
                        mediaPlayer = MediaPlayer.create(applicationContext, R.raw.iron2)
                        mediaPlayer?.start()
                ...
                mediaPlayer!!.setOnCompletionListener {
                    ...
                    Handler().postDelayed({
                        val intent =
                            Intent(this@MultiLevelFour2, MultiLevelFour3::class.java) //ИЗМЕНИТЬ
                        startActivity(intent)
                        finish()
                    }, 500)
                    //ВЫХОД ИЗ ТРЭКА (Задержка 0.3 секунды) - Начало
    ...
    override fun onResume() {
        ...
        if (check <= 1) { //Если свернули игру до нажатия варианта ответа, либо когда нажал только один игрок
            mediaPlayer = MediaPlayer.create(applicationContext, R.raw.iron1)
            mediaPlayer?.start()
    ...
    override fun onBackPressed() {
        if (Yodo1Mas.getInstance().isInterstitialAdLoaded) { //TODO
            Yodo1Mas.getInstance().showInterstitialAd(this@MultiLevelFour2) // показать рекламу
            val intent =
                Intent(this@MultiLevelFour2, GameMenu::class.java) //Создали намерение для перехода
            startActivity(intent)
            finish()
        } else {
            try {
                val intent = Intent(this@MultiLevelFour2, GameMenu::class.java)
                startActivity(intent)
                finish()
            } catch (e: Exception) {
                //Пусто
