   ...
   lateinit var multiscoredraw: TextView
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnclose.setOnClickListener {
            if (Yodo1Mas.getInstance().isInterstitialAdLoaded) { //TODO
                Yodo1Mas.getInstance().showInterstitialAd(this@MultiLevel5)
                cleanResult()
                val intent = Intent(this@MultiLevel5, GameMenu::class.java)
        ...
        btn_back.setOnClickListener {
            ...
            if (Yodo1Mas.getInstance().isInterstitialAdLoaded) { //TODO
                Yodo1Mas.getInstance().showInterstitialAd(this@MultiLevel5)
                val intent =
                    Intent(this@MultiLevel5, GameMenu::class.java) //Создали намерение для перехода
                startActivity(intent)
                finish()
            } else {
                try {
        ...
        p1btn_up_left.setOnClickListener {
            try {
                ...
                if (check == 2) { //Если нажали два раза
                    ...
                    if (mediaPlayer != null) {
                        releaseMP()
                        mediaPlayer = MediaPlayer.create(applicationContext, R.raw.cucumber2)
                        mediaPlayer?.start()
    ...
    override fun onResume() {
        ...
        if (check <= 1) { //Если свернули игру до нажатия варианта ответа, либо когда нажал только один игрок
            mediaPlayer = MediaPlayer.create(applicationContext, R.raw.cucumber1)
            mediaPlayer?.start()
