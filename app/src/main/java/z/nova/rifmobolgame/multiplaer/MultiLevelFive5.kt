   ...
   lateinit var multiscoredraw: TextView
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnclose.setOnClickListener {
            if (Yodo1Mas.getInstance().isInterstitialAdLoaded) { //TODO
                Yodo1Mas.getInstance().showInterstitialAd(this@MultiLevelFive5)
                cleanResult()
                val intent = Intent(this@MultiLevelFive5, GameMenu::class.java)
        ...
        p1btn_up_left.setOnClickListener {
            try {
                ...
                if (check == 2) { //Если нажали два раза
                    ...
                    if (mediaPlayer != null) {
                        releaseMP()
                        mediaPlayer = MediaPlayer.create(applicationContext, R.raw.olesia2)
                        mediaPlayer?.start()
    ...
    override fun onResume() {
        ...
        if (check <= 1) { //Если свернули игру до нажатия варианта ответа, либо когда нажал только один игрок
            mediaPlayer = MediaPlayer.create(applicationContext, R.raw.olesia1)
            mediaPlayer?.start()
    ...
    override fun onBackPressed() {
        if (Yodo1Mas.getInstance().isInterstitialAdLoaded) { //TODO
            Yodo1Mas.getInstance().showInterstitialAd(this@MultiLevelFive5) // показать рекламу
            val intent = Intent(this@MultiLevelFive5, GameMenu::class.java)
            startActivity(intent)
            finish()
        } else {
