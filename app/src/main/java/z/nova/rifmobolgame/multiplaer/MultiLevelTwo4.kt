...
class MultiLevelTwo4 : AppCompatActivity() {
   ...
   lateinit var score2: TextView
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btn_back.setOnClickListener {
            ...
            if (Yodo1Mas.getInstance().isInterstitialAdLoaded) { //TODO
                Yodo1Mas.getInstance().showInterstitialAd(this@MultiLevelTwo4) //Показать рекламу
                val intent = Intent(
                    this@MultiLevelTwo4,
                    GameMenu::class.java
        ...
        p1btn_up_left.setOnClickListener {
            try {
                ...
                if (check == 2) { //Если нажали два раза
                    ...
                    if (mediaPlayer != null) {
                        releaseMP()
                        mediaPlayer = MediaPlayer.create(applicationContext, R.raw.disco2)
                        mediaPlayer?.start()
                ...
                mediaPlayer!!.setOnCompletionListener {
                    p1btn_bottom_right.background =
                        resources.getDrawable(R.drawable.style_btn_stroke_whiteblack_press_whitgreen)
                    p2btn_bottom_right.background =
                        resources.getDrawable(R.drawable.style_btn_stroke_whiteblack_press_whitgreen)
                    accrualOfPoints()
                    //ВЫХОД ИЗ ТРЭКА (Задержка 0.3 секунды) - Начало
                    Handler().postDelayed({
                        val intent = Intent(this@MultiLevelTwo4, MultiLevelTwo5::class.java)
                        startActivity(intent)
                        finish()
                    }, 500)
                    //ВЫХОД ИЗ ТРЭКА (Задержка 0.3 секунды) - Начало
    ...
    override fun onResume() {
        ...
        score2 = findViewById(R.id.scoree2)
        val p1btn_bottom_right =
            findViewById<View>(R.id.p1btn_right_bottom) as Button //Создаём нижнюю левую кнопку
        val p2btn_bottom_right =
            findViewById<View>(R.id.p2btn_right_bottom) as Button //Создаём нижнюю левую кнопку (второй игрок)
        if (check <= 1) { //Если свернули игру до нажатия варианта ответа, либо когда нажал только один игрок
            mediaPlayer = MediaPlayer.create(applicationContext, R.raw.disco1)
            mediaPlayer?.start()
    ...
    override fun onBackPressed() {
        if (Yodo1Mas.getInstance().isInterstitialAdLoaded) { //TODO
            Yodo1Mas.getInstance().showInterstitialAd(this@MultiLevelTwo4) // показать рекламу
            val intent =
                Intent(this@MultiLevelTwo4, GameMenu::class.java) //Создали намерение для перехода
            startActivity(intent)
            finish()
        } else {
            try {
                val intent = Intent(this@MultiLevelTwo4, GameMenu::class.java)
                startActivity(intent)
                finish()
            } catch (e: Exception) {
                //Пусто
