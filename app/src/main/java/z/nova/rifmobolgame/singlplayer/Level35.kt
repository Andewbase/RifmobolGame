...
class Level35 : AppCompatActivity() {
    ...
    private val singlRoundFactory = SinglRoundFactory(35, this@Level35)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btn_up_left.setTextSize(TypedValue.COMPLEX_UNIT_SP, 14f)
        btn_up_right.setTextSize(TypedValue.COMPLEX_UNIT_SP, 14f)
        btn_bottom_left.setTextSize(TypedValue.COMPLEX_UNIT_SP, 18f)
        btn_bottom_right.setTextSize(TypedValue.COMPLEX_UNIT_SP, 18f)
        ...
        singlRoundFactory.dialogWinCreate(this@Level35, PlayGame::class.java) //TODO Установить следующий уровень
        //Вызов диалогового окна Выйграл "В Конце" - конец

        //Вызов диалогового окна Проиграл "В Конце" - начало
        singlRoundFactory.dialogLoseCreate(this@Level35, Level35::class.java, PlayGame::class.java) //TODO Установить повторяющийся, следующий уровень
        //Вызов диалогового окна Проиграл "В Конце" - конец

        //Кнопка "Назад" - начало
        btnBack.setOnClickListener {
            //Обрабатывем нажатие кнопки "Назад" - начало
            singlRoundFactory.clickBackButton(this@Level35)
        }
        //Кнопка "Назад" - конец

        //Обрабатываем нажатие на "Левую Верхнюю Кнопку" - начало
        btn_up_left.setOnClickListener {
            singlRoundFactory.clickButtonRoundLose(1, btn_up_left, btn_bottom_right)
        ...
        btn_up_right.setOnClickListener {
            singlRoundFactory.clickButtonRoundLose(2, btn_up_right, btn_bottom_right)
            //Метод показывающий правельный ответ - конец
        ...
        btn_bottom_left.setOnClickListener {
            singlRoundFactory.clickButtonRoundLose(3, btn_bottom_left, btn_bottom_right)
            //Метод показывающий правельный ответ - конец
    ...
    override fun onResume() {
        super.onResume()
        singlRoundFactory.mediaStart(btn_bottom_right)
    }
    ...
    override fun onDestroy() {
        super.onDestroy()
        singlRoundFactory.releaseMP()
    } //Освобождаем ресурсы проигрывателя при уничтожение активити - коне
}